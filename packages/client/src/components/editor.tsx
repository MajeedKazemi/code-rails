                    ...
                    if (response.ok) {
                        response.json().then((data) => {
                            ...
                            editor.onDidChangeModelContent((e) => {
                                // after 10s of inactivity, check using tree-sitter for syntax errors
                                // if error and has changed compared to the previous state of code [the diff to be a few characters]
                                // if different -> show a button: "need help fixing this?" -> use LLM-API to suggest incremenetal fixes
    ...
    useEffect(() => {
        socket?.on("python", (data: any) => {
            ...
            if (data.type === "stderr") {
                ...
                // on error -> run llm-api to help user fix the error
                // show a button about "need help fixing this?" -> will annotate the code with suggested fixes.
                // it is important to compare it with previous state of the code to avoid suggesting the same fix multiple times: previous suggestion + previous code should be stored.
            }
            if (data.type === "close") {
                setRunning(false);
                setRunId(runId + 1);
                ...
                // on finish:
                // first compares the code from the previous code state
                // if changed -> then run LLM-API
            }
        });
    }, [output, runId]);
